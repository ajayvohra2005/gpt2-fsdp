...
class GP2SelfAttention(nn.Module):
...
class GPT2TransformerBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.layer.append(GP2SelfAttention(config))
        self.layer.append(GPT2MLP(config))
...
class GPT2(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            h = nn.ModuleList([GPT2TransformerBlock(config) for _ in range(config.n_layer)]),
    ...
    def from_hf_model(self, hf_model:str):
        ...
        model_config['bias'] = True # always True for GPT model checkpoints
        ...
        model_hf = GPT2LMHeadModel.from_pretrained(hf_model)
        ...
        for k in hf_state_dict_keys:
            ...
            if re.match(r'^(transformer\.h\.[0-9]+)\.(ln_1.+)$', k):
                k_ = re.sub(r'^(transformer\.h\.[0-9]+)\.(ln_1.+)$', r'\1.layer.0.\2', k )
            elif re.match(r'^(transformer\.h\.[0-9]+)\.(ln_2.+)$', k):
                k_ = re.sub(r'^(transformer\.h\.[0-9]+)\.(ln_2.+)$', r'\1.layer.1.\2', k )
            elif re.match(r'^(.+)(\.attn\.)(.+)$', k):
                k_ = re.sub(r'^(.+)(\.attn\.)(.+)$', r'\1.layer.0.\3', k )
            elif re.match(r'^(.+)(\.mlp\.)(.+)$', k):
                k_ = re.sub(r'^(.+)(\.mlp\.)(.+)$', r'\1.layer.1.\3', k )
            else:
                k_ = k

            if any(k.endswith(w) for w in transposed):
                # special treatment for the Conv1D weights we need to transpose
                assert hf_state_dict[k].shape[::-1] == state_dict[k_].shape
                with torch.no_grad():
                    state_dict[k_].copy_(hf_state_dict[k].t())
            else:
                # vanilla copy over the other parameters
                assert hf_state_dict[k].shape == state_dict[k_].shape
                with torch.no_grad():
                    state_dict[k_].copy_(hf_state_dict[k])
